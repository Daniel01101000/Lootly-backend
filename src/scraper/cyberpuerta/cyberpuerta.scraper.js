...
const { buildSearchUrl, parseSearchResults } = require('./cyberpuerta.parser');
...
class CyberPuertaScraper extends BaseScraper {
  constructor() {
    super('CyberPuerta', 'https://www.cyberpuerta.mx');
  ...
  async searchProducts({ category, query } = {}) {
    if (query) {
      ...
      return raw.map((p) => normalizeProduct({ ...p, category: category || 'component' }, 'CyberPuerta')).filter(Boolean);
    }
    ...
    for (const query of catConfig.queries) {
      ...
      try {
        ...
        for (const item of raw) {
          if (products.length >= this.maxProductsPerCategory) break;
          const normalized = normalizeProduct({ ...item, category: catConfig.category }, 'CyberPuerta');
          if (normalized) products.push(normalized);
        }
      } catch (err) {
        console.error(`[CyberPuertaScraper] Error searching "${query}": ${err.message}`);
      }
    }
    console.log(`[CyberPuertaScraper] Found ${products.length} products for category "${catConfig.category}"`);
    return products;
  }
}

module.exports = CyberPuertaScraper;
