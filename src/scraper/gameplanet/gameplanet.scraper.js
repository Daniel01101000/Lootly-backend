...
const { buildSearchUrl, parseSearchResults } = require('./gameplanet.parser');
...
const PRODUCT_CATEGORIES = [
  ...
  { category: 'games', queries: ['videojuegos ps5', 'videojuegos xbox', 'videojuegos nintendo', 'juegos pc', 'videojuego'] },
...
class GamePlanetScraper extends BaseScraper {
  constructor() {
    super('GamePlanet', 'https://www.gameplanet.com.mx');
  ...
  async searchProducts({ category, query } = {}) {
    if (query) {
      ...
      return raw.map((p) => normalizeProduct({ ...p, category: category || 'games' }, 'GamePlanet')).filter(Boolean);
    }
    ...
    for (const query of catConfig.queries) {
      ...
      try {
        ...
        for (const item of raw) {
          if (products.length >= this.maxProductsPerCategory) break;
          const normalized = normalizeProduct({ ...item, category: catConfig.category }, 'GamePlanet');
          if (normalized) products.push(normalized);
        }
      } catch (err) {
        console.error(`[GamePlanetScraper] Error searching "${query}": ${err.message}`);
      }
    }
    console.log(`[GamePlanetScraper] Found ${products.length} products for category "${catConfig.category}"`);
    return products;
  }
}

module.exports = GamePlanetScraper;
