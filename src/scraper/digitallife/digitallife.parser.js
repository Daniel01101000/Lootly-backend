...
const SEARCH_URL_TEMPLATE = 'https://www.digitallife.com.mx/catalogsearch/result/?q={query}';
...
const parseSearchResults = (html) => {
  ...
  $('.product-item, .item, [class*="producto"], .product').each((_, element) => {
    try {
      const $el = $(element);

      const nameEl = $el.find('.product-item-link, a.product-item-link, .name a, h2 a');
      let name = nameEl.text().trim();
      let url = nameEl.attr('href') || '';

      if (!name) {
        const altEl = $el.find('a[class*="product"], a[title]');
        name = altEl.attr('title') || altEl.text().trim();
        url = altEl.attr('href') || url;
      }
      if (!name) return;

      if (url && !url.startsWith('http')) {
        url = 'https://www.digitallife.com.mx' + (url.startsWith('/') ? url : '/' + url);
      }

      const imageUrl = extractImage($el);

      let currentPrice = null;
      const priceEl = $el.find('.price, .special-price .price, .normal-price .price, [class*="precio"]');
      ...
      const oldPriceEl = $el.find('.old-price .price, [class*="old"]');
