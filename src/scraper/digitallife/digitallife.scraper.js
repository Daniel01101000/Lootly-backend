...
const { buildSearchUrl, parseSearchResults } = require('./digitallife.parser');
...
class DigitalLifeScraper extends BaseScraper {
  constructor() {
    super('Digital Life', 'https://www.digitallife.com.mx');
  ...
  async searchProducts({ category, query } = {}) {
    if (query) {
      ...
      return raw.map((p) => normalizeProduct({ ...p, category: category || 'component' }, 'Digital Life')).filter(Boolean);
    }
    ...
    for (const query of catConfig.queries) {
      ...
      try {
        ...
        for (const item of raw) {
          if (products.length >= this.maxProductsPerCategory) break;
          const normalized = normalizeProduct({ ...item, category: catConfig.category }, 'Digital Life');
          if (normalized) products.push(normalized);
        }
      } catch (err) {
        console.error(`[DigitalLifeScraper] Error searching "${query}": ${err.message}`);
      }
    }
    console.log(`[DigitalLifeScraper] Found ${products.length} products for category "${catConfig.category}"`);
    return products;
  }
}

module.exports = DigitalLifeScraper;
