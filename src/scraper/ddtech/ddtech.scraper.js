...
const { buildSearchUrl, parseSearchResults } = require('./ddtech.parser');
...
class DDTechScraper extends BaseScraper {
  constructor() {
    super('DDTech', 'https://ddtech.mx');
  ...
  async searchProducts({ category, query } = {}) {
    if (query) {
      ...
      return raw.map((p) => normalizeProduct({ ...p, category: category || 'component' }, 'DDTech')).filter(Boolean);
    }
    ...
    for (const query of catConfig.queries) {
      ...
      try {
        ...
        for (const item of raw) {
          if (products.length >= this.maxProductsPerCategory) break;
          const normalized = normalizeProduct({ ...item, category: catConfig.category }, 'DDTech');
          if (normalized) products.push(normalized);
        }
      } catch (err) {
        console.error(`[DDTechScraper] Error searching "${query}": ${err.message}`);
      }
    }
    console.log(`[DDTechScraper] Found ${products.length} products for category "${catConfig.category}"`);
    return products;
  }
}

module.exports = DDTechScraper;
